...
export async function handleGithubURL(
  ...
  try {
    ...
    const href = `${procUrl}/parser/url?url=${encodedGithubUrl}`;
...
export async function plotSourceData(
  ...
  try {
    ...
    const procUrl = initRequest(cell);
    const baseApi = `${procUrl}/plotter/json`;
...
export async function plotGithubUrl(
  ...
  try {
    ...
    const procUrl = initRequest(cell);
    const encodedGithubUrl = encodeURIComponent(url);
    const href = `${procUrl}/plotter/url?url=${encodedGithubUrl}`;
    const plotOptions = {
      palette_id: 0,
      layout: 'Spring',
      type: 'd3',
    ...
    const data = await PostJsonRequest(href, body);
...
export async function plotUploadedFile(
  ...
  try {
    ...
    // Construct request data
    const procUrl = initRequest(cell);
    //const urlParserRaw = `${procUrl}/parser/file`;
    const urlParserRaw = `${procUrl}/plotter/raw`;
    const file = cell.state.uploaded_files[0];
    ...
    // // Handle response
    // const plotData: Object = await PostJsonRequest(`${procUrl}/plotter/file`, {
    //   name: file.name,
    //   graph: fileGraph,
    // });
    // if (!plotData) {
