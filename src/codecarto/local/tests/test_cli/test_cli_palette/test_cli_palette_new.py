    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        def assert_result(expected, actual):
            try:
                ...
                raise AssertionError(
                    f"Error in test_cli_palette_new.py: "
                    f"\n\nExpected:\n---------\n{expected}\n"
                    f"\n\nActual:\n-------\n{actual}\n"
                )
        ...
        def check_new_data(command):
            ...
            for command in commands:
                ...
                assert_result(
                    round(0.1 * int(command[9]), ndigits=1), palette_data["alphas"]
                )
        ...
        def check_output(command, result, input: str = ""):
            ...
            if input == "":
                ...
                assert_result(
                    (
                        f"New theme '{command[3]}' created with parameters: "
        ...
        set_config_prop("palette_path", os.path.join(temp_dir, temp_file_name))
