class Processor:
    ...
    def process(
        ...
        graph = Parser.parse_list(source_files)
        ...
        if graph and graph.number_of_nodes() > 0:
            ...
            if not from_api:
                ...
                paths = create_output_dirs(make_dir=True)
                ...
                if do_plot:
                    ...
                    if (
                        do_json
                    ):  # this is asking if we should convert back from json to graph
                        # Create a json file of the graph
                        json_graph_file = PolyGraph.graph_to_json_file(
                            graph, paths["json_graph_file_path"]
                        )
                        json_graph = PolyGraph.json_file_to_graph(json_graph_file)
                    ...
                    save_json(graph, paths["json_graph_file_path"])
