...
class BaseASTVisitor(ast.NodeVisitor):
    ...
    def create_node(self, label, node_type, module, parent=None):
        ...
        if not self.graph.has_node(node_id):
            self.graph.add_node(
                node_id, label=label, type=node_type, module=module, parent=parent
            )
        if parent:
        ...
        for import_name, full_name in self.imports.items():
            ...
            if full_name in self.graph:
                import_node_id = self.create_node(
                    import_name, "Import", self.current_module_name
                )
...
class PythonAST(BaseASTVisitor):
    ...
    def visit_Module(self, node):
        module_id = self.create_node(
            self.current_module_name, "Module", self.current_module_name
        )
    ...
    def visit_ImportFrom(self, node):
        ...
        for alias in node.names:
            ...
            self.graph.add_edge(
                f"{self.current_module_name}.{self.current_module_name}", import_node_id
            )
            self.graph.add_edge(import_node_id, f"{node.module}.{node.module}")

    def visit_ClassDef(self, node):
        class_id = self.create_node(
            node.name, "Class", self.current_module_name, parent=self.current_parent_id
        )
        ...
        for child in node.body:
            if isinstance(child, ast.FunctionDef):
                func_id = self.create_node(
                    child.name, "Function", self.current_module_name, parent=class_id
                )
    ...
    def visit_Name(self, node):
        ...
        name_id = self.create_node(
            node.id, "Name", self.current_module_name, parent=self.current_parent_id
        )
        return name_id
    ...
    def visit_For(self, node):
        for_id = self.create_node(
            "ForLoop", "For", self.current_module_name, parent=self.current_parent_id
        )
        previous_parent_id = self.current_parent_id
    ...
    def visit_Constant(self, node):
        ...
        const_id = self.create_node(
            label, "Constant", self.current_module_name, parent=self.current_parent_id
        )
        return const_id
