...
from codecarto import Parser, PolyGraph, ErrorHandler
...
@router.post("/parser/parse", request_body_max=1000000, response_class=JSONResponse, responses={200: {"content": {"application/json": {}}}})
async def parse(
    ...
    if not source_files:
        ErrorHandler.log(caller="API/parser/parse", break_line = 28, msg="HTTP Status:400 - No source files provided.")
        ...
        try: 
            ...
            for file in source_files:
                # Check file sizes
                if file.file._file.tell() > 1000000: # Limit file size to 1MB for this example
                    msg:str =f"HTTP Status:413 - File size for {file.filename} exceeds the limit of 1MB."
                    ErrorHandler.log(caller="API/parser/parse", break_line = 40, msg=msg)
            ...
            if not passed_files or len(passed_files) == 0:
                ...
                ErrorHandler.log(caller="API/parser/parse", break_line = 50, msg=msg)
                raise HTTPException(status_code=415, detail=msg)

            # Parse the source code and return the {json_data, output_dir} dict 
            graph = Parser.parse_source_files([file.file for file in passed_files])
            json_data = PolyGraph.graph_to_json_data(graph)  

            # Return the json data
            return JSONResponse(content=json_data) # Serializes the content to a JSON string.
        except Exception as e:
            # internal server error log
            ErrorHandler.log(exception=e, caller="API/parser/parse", msg="Failed to parse source code.")
