...
from .routers import palette_router, parser_router, plotter_router, polygraph_router, processor_router
from codecarto import LogHandler
...
async def log_duration(request: Request, call_next):
    ...
    LogHandler.log_duration(path=path, start=start_time, end=end_time, duration=duration)
    return response
