...
def parse_line(line: str) -> Optional[PamEvent]:
    ...
    if m:
        ...
        service = m['service'].split('(')[0].rstrip(':')
        pid = int(m['pid'])
        body = m['body']
    else:
        # Try journald format
    ...
    if _PAM_AUTH_OPEN.search(body):
        ...
        ev.event_type = (
            EventType.SSSD_CALL
            if not _PAM_SUCCESS.search(body) and not _PAM_FAIL.search(body)
            else EventType.SSSD_RESULT
        )
    elif _KRB_TGT.search(body):
        ev.event_type = (
            EventType.KRB_CALL
            if 'request' in body.lower() or 'AS-REQ' in body
            else EventType.KRB_RESULT
        )
    elif ev.module:
